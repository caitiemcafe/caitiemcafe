...
function add() {
  ...
  cart.add(props.product, quantity.value, notes.value)
  emit('close')
}
</script>
    ...
    <section class="product-modal card" role="dialog" aria-modal="true" aria-labelledby="product-title">
      ...
      <div class="modal-body">
        ...
        <div class="quantity-row">
          <span>Số lượng</span>
          <div class="stepper">
            <button aria-label="Giảm" @click="quantity = Math.max(1, quantity - 1)"><Minus :size="17" /></button>
            <b>{{ quantity }}</b>
            <button aria-label="Tăng" @click="quantity = Math.min(30, quantity + 1)"><Plus :size="17" /></button>
          </div>
        </div>
