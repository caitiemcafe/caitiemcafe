...
const qrDataUrl = ref('')
const vibeUrl = ref(`${window.location.origin}/vibe`)

async function renderQrCode() {
  await nextTick()
  try {
    // Render on-screen preview canvas
    if (qrCanvas.value) {
    ...
    // Generate data URL for 100% reliable printing
...
async function downloadQrCode() {
  try {
    ...
    // Construct clean composite canvas with QR code on top and text below
...
<template>
  <section class="quotes-page">
    <!-- Printable QR Card Container (Visible ONLY during browser print) -->
    ...
    <!-- Shared Static Vibe QR Print Section -->
...
.ai-box {
  padding: 20px;
  display: grid;
  grid-template-columns: 55px minmax(180px, 0.8fr) 1fr 85px auto;
  gap: 14px;
  align-items: center;
}
.ai-icon {
