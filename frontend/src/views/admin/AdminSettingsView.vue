  ...
  <section class="settings-page">
    ...
    <form class="settings-form" @submit.prevent="save">
      <div class="settings-grid">
        <!-- Panel 1: Thông tin quán -->
        <section class="admin-card panel">
          <h2 class="serif">Thông tin liên hệ</h2>
          <label class="field-label">Tên quán
            <input v-model="form.shop_name" class="field" required maxlength="120" />
          </label>
          <label class="field-label">Số điện thoại
            <input v-model="form.shop_phone" class="field" maxlength="30" />
          </label>
          <label class="field-label">Địa chỉ
            <textarea v-model="form.shop_address" class="field" rows="3" maxlength="500"></textarea>
          </label>
          <label class="field-label">Email hiển thị
            <input v-model="form.shop_email" class="field" type="email" maxlength="254" />
          </label>
        </section>

        <!-- Panel 2: Nhận đơn & Bảo mật -->
        <section class="admin-card panel">
          <h2 class="serif">Nhận đơn & Giao hàng</h2>
          <label class="field-label">Phí giao hàng mặc định (đ)
            <input v-model.number="form.shipping_fee" class="field" type="number" min="0" step="1000" />
          </label>
      ...
      <!-- Panel 3: AI Configuration Card (Matching Warm Coffee Light Theme) -->
      <section class="admin-card panel ai-card">
          ...
          <label class="field-label">API KEY
            <div class="api-key-input-wrap">
              <input
                ...
                class="field"
                placeholder="Để trống sẽ dùng GEMINI_API_KEY trong .env"
              />
        ...
        <!-- Proxy URL optional input -->
        <label class="field-label">PROXY URL (TÙY CHỌN - VÍ DỤ: HTTPS://API.OPENAI-PROXY.COM)
          <input
            v-model="form.ai_proxy_url"
            type="text"
            class="field"
            placeholder="https://vertex-key.com/api/v1 (để trống nếu dùng endpoint mặc định)"
          />
        </label>

        <!-- Model select + Fetch button -->
        <label class="field-label">MODEL SỬ DỤNG
          <div class="model-select-row">
            <select v-model="form.ai_model" class="field flex-1">
              <option v-for="m in availableModels" :key="m" :value="m">{{ m }}</option>
              <option v-if="!availableModels.includes(form.ai_model)" :value="form.ai_model">
                {{ form.ai_model }}
              </option>
            </select>
            <button
              type="button"
              class="btn-fetch-model"
        ...
        <div class="ai-playground-section">
          ...
          <div class="playground-input-row">
            <textarea
              ...
              rows="2"
              class="field flex-1"
              placeholder="Nhập prompt câu lệnh thử nghiệm AI..."
            ></textarea>
            <button
              type="button"
              class="btn btn-primary btn-test-action"
              :disabled="testingAi"
              @click="runAiTest"
            >
...
.btn-fetch-model {
  ...
  padding: 12px 18px;
  background: #ede2d5;
  border: 1px solid rgba(59, 36, 23, 0.16);
  border-radius: 12px;
  color: var(--coffee);
  font-weight: 600;
  font-size: 0.88rem;
...
.btn-fetch-model:hover:not(:disabled) {
  background: #e4d3c3;
  border-color: var(--coffee);
}
.btn-fetch-model:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
