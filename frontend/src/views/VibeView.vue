...
async function draw() {
  ...
  error.value = ''
  try {
    const { data } = await api.get<ApiResponse<Quote>>('/vibe/random')
    if (data?.data) {
      localQuote.value = data.data
      const today = getTodayString()
      isAlreadyDrawnToday.value = true
      try {
        localStorage.setItem(VIBE_STORAGE_KEY, JSON.stringify({ date: today, quote: data.data }))
      } catch (e) {
        // ignore storage error
      }
    } else {
      error.value = 'Chưa lấy được thông điệp.'
    }
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Chưa lấy được thông điệp.'
  } finally {
    loading.value = false
  }
}
...
async function share() {
  const data = { title: 'Một lời nhắn từ Cái Tiệm', text: quote.value?.content || '', url: window.location.href }
...
<template>
  <main class="vibe-page">
    <div class="grain"></div>
    <RouterLink to="/" class="back"><ArrowLeft :size="18" /> Về Cái Tiệm</RouterLink>
    ...
    <section class="vibe-shell">
      ...
      <div class="quote-card" :key="quote?.id || 0">
        ...
        <div v-if="loading" class="loading"><span class="spinner"></span><p>Đang tìm một câu thật hợp...</p></div>
        <div v-else-if="error" class="loading"><p>{{ error }}</p><button class="btn btn-outline" @click="draw">Thử lại</button></div>
        <template v-else-if="quote"><blockquote class="serif">{{ quote.content }}</blockquote><span class="topic">{{ quote.topic || 'Từ Cái Tiệm' }}</span></template>
      </div>
      ...
      <div class="actions">
        ...
        <button v-else class="btn btn-primary" :disabled="loading || !quote" @click="draw">
          <Sparkles :size="17" /> Lời nhắn hôm nay
        </button>

        <button class="btn btn-outline" :disabled="!quote" @click="share">
          <Copy v-if="copied" :size="17" /><Share2 v-else :size="17" />{{ copied ? 'Đã sao chép' : 'Chia sẻ' }}
        </button>
      </div>
...
.daily-notice { display: flex; align-items: center; gap: 10px; margin-top: 16px; padding: 12px 20px; border-radius: 16px; background: rgba(133, 75, 44, 0.08); border: 1px solid rgba(133, 75, 44, 0.2); color: #613318; font-size: 0.88rem; line-height: 1.45; text-align: left; animation: rise .4s both; }
...
.footnote { max-width: 470px; margin: 28px 0 0; color: rgba(59,36,23,.65); font-size: .78rem; line-height: 1.6; }.rotating { animation: spin .7s linear infinite; }
