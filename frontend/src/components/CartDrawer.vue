...
import { CheckCircle2, MapPin, Minus, Plus, ShoppingBag, Trash2, X } from '@lucide/vue'
...
function saveProfile(name: string, phone: string, email: string, address: string) {
  ...
  const cleanName = name.trim(); const cleanPhone = phone.trim(); const cleanEmail = email.trim(); const cleanAddress = address.trim()
  ...
  const existingIdx = profiles.findIndex((p) => p.customerPhone === cleanPhone && p.customerAddress.toLowerCase() === cleanAddress.toLowerCase())
  const updatedProfile: CustomerProfile = {
    id: existingIdx >= 0 ? profiles[existingIdx].id : crypto.randomUUID(),
    customerName: cleanName, customerPhone: cleanPhone, customerEmail: cleanEmail, customerAddress: cleanAddress
  }
  if (existingIdx >= 0) profiles.splice(existingIdx, 1)
...
async function submit() {
  ...
  try {
    ...
    cart.clear(); cart.isOpen = true; step.value = 'success'
      ...
      <form v-else-if="step === 'checkout'" class="checkout" @submit.prevent="submit">
        <div v-if="savedProfiles.length" class="saved-profiles-section">
          ...
          <div class="profile-chips">
            <button v-for="p in savedProfiles" :key="p.id" type="button" :class="['profile-chip', { active: selectedProfileId === p.id }]" @click="applyProfile(p)">
...
.saved-profiles-section { display: grid; gap: 8px; padding: 12px 14px; border-radius: 16px; background: rgba(113, 80, 56, 0.06); border: 1px dashed rgba(113, 80, 56, 0.22); }
.profiles-label { display: flex; align-items: center; gap: 6px; font-size: 0.76rem; font-weight: 700; color: #794426; text-transform: uppercase; letter-spacing: 0.05em; }
.profile-chips { display: flex; flex-wrap: wrap; gap: 8px; }
.profile-chip { display: flex; flex-direction: column; align-items: flex-start; gap: 2px; padding: 8px 12px; border-radius: 12px; border: 1px solid rgba(59, 36, 23, 0.15); background: white; color: var(--coffee); cursor: pointer; text-align: left; transition: all 0.2s ease; max-width: 100%; }
.profile-chip:hover { border-color: #a45f2e; background: #fffbf6; }
.profile-chip.active { border-color: #a45f2e; background: #fdf5ec; box-shadow: 0 2px 8px rgba(164, 95, 46, 0.15); }
.chip-title { font-size: 0.78rem; font-weight: 700; }
.chip-addr { font-size: 0.73rem; color: #796a60; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 220px; }
