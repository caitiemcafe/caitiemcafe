...
function orderHtml(order: OrderWithItems, heading: string) {
  ...
  return `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#2b211b"><h1 style="color:#6f3d24">${heading}</h1><p><b>Mã đơn:</b> ${escapeHtml(order.orderCode)}</p><p><b>Khách:</b> ${escapeHtml(order.customerName)} — ${escapeHtml(order.customerPhone)}</p><p><b>Địa chỉ:</b> ${escapeHtml(order.customerAddress)}</p>${order.notes ? `<p><b>Ghi chú:</b> ${escapeHtml(order.notes)}</p>` : ''}<table style="width:100%;border-collapse:collapse">${rows}<tr style="border-top:1px solid #ddd"><td style="padding-top:12px"><b>Tổng cộng</b></td><td style="padding-top:12px;text-align:right"><b>${money.format(Number(order.totalAmount))}</b></td></tr></table><p style="margin-top:24px;color:#6b625d">Cảm ơn bạn đã chọn Cái Tiệm KàFe.</p></div>`;
}
